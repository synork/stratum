...
import { helperDomains, HomeAssistantClient, type HelperDomain } from "./home-assistant.js";
...
app.get("/api/health", async () => ({
  status: "ok" as const,
  homeAssistant: !homeAssistant.configured ? "unconfigured" as const : homeAssistant.connected ? "connected" as const : "unavailable" as const,
...
app.post("/api/providers", async (request, reply) => {
  ...
  if (provider.models.length === 0) {
    try { provider.models = await discoverModels(provider); }
    catch (error) { app.log.warn({ error, provider: provider.kind }, "Automatic model discovery failed"); }
  }
...
app.get("/api/entities", async (request) => {
  const query = z.object({ q: z.string().default(""), limit: z.coerce.number().int().min(1).max(200).default(60) }).parse(request.query);
  return { entities: homeAssistant.search(query.q, query.limit), total: homeAssistant.count };
});

app.get("/api/proposals", async () => database.listProposals());

app.get<{ Params: { type: string; resourceId: string } }>("/api/resources/:type/:resourceId/current", async (request, reply) => {
  ...
  try {
    ...
    if (type === "helper") {
      ...
      if (!helperDomains.includes(domain as HelperDomain)) return reply.code(400).send({ error: "Unknown helper domain" });
      return await homeAssistant.getHelper(domain as HelperDomain, id);
    }
    return reply.code(400).send({ error: "Unknown resource type" });
  } catch (error) {
    if (error instanceof Error && error.message.includes("404")) return reply.code(404).send({ error: "Resource not found" });
    throw error;
  }
});

const threadStateSchema = z.object({
  id: z.string(), title: z.string().max(200), mode: z.enum(["plan", "build"]), prompt: z.string().max(30_000),
  transcript: z.array(z.object({
    id: z.string(), role: z.enum(["user", "assistant", "reasoning", "activity"]), text: z.string().max(120_000),
    detail: z.string().max(40_000).optional(), status: z.enum(["running", "complete", "error"]).optional(),
  })).max(500),
  entityIds: z.array(z.string()).max(100), createdAt: z.string(),
  contextSummary: z.string().max(80_000).optional(), compactedThrough: z.number().int().min(0).optional(),
});

app.get("/api/threads", async () => database.listThreads().map((thread) => ({
  id: thread.id, title: thread.title, archived: thread.archived, createdAt: thread.createdAt, updatedAt: thread.updatedAt,
})));
...
app.post("/api/memories", async (request, reply) => {
  const input = z.object({ content: z.string().min(1).max(4_000), tags: z.array(z.string().max(50)).max(20).default([]) }).parse(request.body);
...
const compactInput = z.object({
  providerId: z.string(), modelId: z.string(), existingSummary: z.string().max(80_000).default(""),
  messages: z.array(z.object({ role: z.enum(["user", "assistant"]), content: z.string().max(120_000) })).min(1).max(200),
});
app.post<{ Params: { id: string } }>("/api/threads/:id/compact", async (request, reply) => {
  ...
  if (!provider || !provider.enabled || !provider.models.includes(input.modelId)) return reply.code(400).send({ error: "Select a configured model" });
...
app.post<{ Params: { id: string } }>("/api/proposals/:id/approve", async (request, reply) => {
  ...
  if (proposal.type === "automation") proposal.validation = homeAssistant.validateAutomation(proposal.payload);
    ...
    if (!helperDomains.includes(domain as HelperDomain) || !helperConfig || typeof helperConfig !== "object" || Array.isArray(helperConfig)) return reply.code(422).send({ error: "Invalid helper proposal" });
    proposal.validation = homeAssistant.validateHelper(domain as HelperDomain, helperConfig as Record<string, unknown>);
  }
  if (!proposal.validation.valid) { database.saveProposal(proposal); return reply.code(422).send({ error: proposal.validation.errors.join("; ") }); }
  proposal.status = "approved";
  database.saveProposal(proposal);
  try {
    const before = proposal.type === "automation" ? await homeAssistant.getAutomation(proposal.resourceId) : proposal.type === "dashboard" ? await homeAssistant.getDashboard(proposal.resourceId) : null;
    if (proposal.type === "automation") await homeAssistant.publishAutomation(proposal.resourceId, proposal.payload);
    else if (proposal.type === "dashboard") await homeAssistant.publishDashboard(proposal.resourceId, proposal.payload);
    else {
...
const dashboardReviewInput = z.object({ providerId: z.string(), modelId: z.string(), dashboardPath: z.string().startsWith("/").refine((path) => !path.includes("\\") && !path.startsWith("//"), "Dashboard path must remain on Home Assistant") });
...
const chatInput = z.object({
  providerId: z.string(), modelId: z.string(), mode: z.enum(["plan", "build"]),
  prompt: z.string().min(1).max(30_000),
  history: z.array(z.object({ role: z.enum(["user", "assistant"]), content: z.string().max(30_000) })).max(100).default([]),
...
app.post("/api/chat", async (request, reply) => {
  ...
  reply.raw.once("close", () => { if (!reply.raw.writableEnded) abortController.abort(); });
  ...
  const result = runAgent(provider, input.modelId, input.mode, input.prompt, input.history, input.contextSummary, input.contextEntityIds, homeAssistant, database, research, abortController.signal);
  ...
  const detail = (value: unknown) => {
    try { return JSON.stringify(value, null, 2).slice(0, 40_000); }
    catch { return String(value).slice(0, 40_000); }
  };
  ...
  try {
    for await (const part of result.stream) {
      if (part.type === "text-start") reply.raw.write(`data: ${JSON.stringify({ type: "text-start", id: eventId("text", part.id, textIds) })}\n\n`);
      else if (part.type === "text-delta") reply.raw.write(`data: ${JSON.stringify({ type: "text", id: eventId("text", part.id, textIds), delta: part.text })}\n\n`);
      else if (part.type === "text-end") { reply.raw.write(`data: ${JSON.stringify({ type: "text-end", id: eventId("text", part.id, textIds) })}\n\n`); textIds.delete(part.id); }
      else if (part.type === "reasoning-start") reply.raw.write(`data: ${JSON.stringify({ type: "reasoning-start", id: eventId("reasoning", part.id, reasoningIds) })}\n\n`);
      else if (part.type === "reasoning-delta") reply.raw.write(`data: ${JSON.stringify({ type: "reasoning", id: eventId("reasoning", part.id, reasoningIds), delta: part.text })}\n\n`);
      else if (part.type === "reasoning-end") { reply.raw.write(`data: ${JSON.stringify({ type: "reasoning-end", id: eventId("reasoning", part.id, reasoningIds) })}\n\n`); reasoningIds.delete(part.id); }
      else if (part.type === "tool-input-start") reply.raw.write(`data: ${JSON.stringify({ type: "tool", state: "running", tool: part.toolName, callId: `${runId}-${part.id}` })}\n\n`);
      else if (part.type === "tool-call") reply.raw.write(`data: ${JSON.stringify({ type: "tool", state: "running", tool: part.toolName, callId: `${runId}-${part.toolCallId}`, detail: detail(part.input) })}\n\n`);
      else if (part.type === "tool-result") reply.raw.write(`data: ${JSON.stringify({ type: "tool", state: "complete", tool: part.toolName, callId: `${runId}-${part.toolCallId}`, detail: detail(part.output) })}\n\n`);
      else if (part.type === "tool-error") reply.raw.write(`data: ${JSON.stringify({ type: "tool", state: "error", tool: part.toolName, callId: `${runId}-${part.toolCallId}`, detail: detail(part.error) })}\n\n`);
      else if (part.type === "error") reply.raw.write(`data: ${JSON.stringify({ type: "error", message: String(part.error) })}\n\n`);
      else if (part.type === "abort") reply.raw.write(`data: ${JSON.stringify({ type: "abort" })}\n\n`);
    }
...
if (homeAssistant.configured) {
  homeAssistant.refresh().catch((error) => app.log.warn({ error }, "Initial Home Assistant inventory failed"));
  setInterval(() => homeAssistant.refresh().catch((error) => app.log.warn({ error }, "Home Assistant inventory refresh failed")), 60_000).unref();
}
...
try {
  ...
  await app.register(fastifyStatic, {
    ...
    setHeaders(res, path) {
      ...
      if (path.endsWith(".html")) serverResponse.setHeader("cache-control", "no-cache, no-store, must-revalidate");
      else if (/[.-][A-Za-z0-9_-]{8}\.(js|css|woff2?)$/.test(path)) serverResponse.setHeader("cache-control", "public, max-age=31536000, immutable");

      ...
      <aside className={`evidence-panel ${inspectorOpen ? "evidence-panel--open" : ""}`}>
        ...
        {proposals.length === 0 ? <div className="evidence-empty"><Gauge size={23} /><h3>No draft yet</h3><p>Generated automation or dashboard configurations and visual-review results appear here.</p></div> : proposals.map((proposal) => (
          <article className="proposal-card" key={proposal.id}>
            ...
            {(proposal.status === "rejected" || proposal.status === "failed") && <div className="proposal-actions">
              {proposal.status === "failed" && <button type="button" onClick={async () => { try { await api.retryProposal(proposal.id); setProposals(await api.proposals()); showToast("Draft reset for retry", "info"); } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); } }}>Retry</button>}
              <button type="button" onClick={async () => { try { await api.deleteProposal(proposal.id); setProposals(await api.proposals()); } catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)); } }}>Remove record</button>
            </div>}
          </article>
        ))}
